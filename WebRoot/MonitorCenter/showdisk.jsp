						...
						int hostId=hInfo.getId();
			 			...
			 			<li>
							...
							<ul class="<%=reqhostId==null&&hostId!=host.getFirstHostInfo().getId()||reqhostId!=null&&Integer.parseInt(reqhostId)!=hostId?"hide":"" %>">
								...
								<li>
									<a href="showdisk.jsp?hostId=<%=hostId %>" class="<%=hostId==Integer.valueOf(reqhostId)?"current":"" %>">Disk</a>
								</li>
				...
				<div class="content_data_table">
					...
					<%for(Iterator<DiskInfo> it=hInfo.getDisk().iterator();it.hasNext();){
					 ...
					 %>
						<tr>
							<td>
								<img id="show" src="GetDiskInfo?hostId=<%=reqhostId %>&name=<%=disk.getName() %>&time=1"/>
							</td>
						</tr>
					<%}%>
					</table>
