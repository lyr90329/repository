...
　　						<input type=file size="30" value="upload your own Web Service" name="s" onfocus="if (this.value == 'upload your own Web Service') {this.value = '';}" onblur="if (this.value == '') {this.value = 'upload your own Web Service';}"/>
...
<%
 ...
 <%
List<UploadBpmn> list2=bpmnDao.findUploadBpmn("userName",userName);
UploadBpmn uploadbpmn;
if(null!=list2){
	for(int i=0;i < list2.size(); i++){
		uploadbpmn = list2.get(i);
		BpmnDao dao2 = new BpmnDao();
		UploadBpmn trial2;
		trial2 = dao2.findByBPMNIdAndUserName2(uploadbpmn.getBpmnId(), userName);
		if (trial2.getDeployStatus().equals("true")) {					
					System.out.println("BPMN "+trial2.getBpmnName()+" has been deployed.");
				} else {					
					try
					{
						String bpmnName=trial2.getBpmnName()+".bpmn";
			            bpmnName="bpmn/"+userName+"/"+bpmnName;
						DeploymentProxyClient c = new DeploymentProxyClient(
						Config.getDeployService());
				         if(c.deploy(Config.getDeployService(),Config.getPath(bpmnName)))
				         {
				           System.out.println("BPMN "+trial2.getBpmnName()+" has been deployed.");
				         }
				         else
				         {
				            System.out.println("BPMNEngine initialization fails!");
				         }
					} catch (Exception e) {
						e.printStackTrace();
					}
...
%>
		<div class="excrept_post">
    		...
    		<!-- Thumbnail from Custom Field, Post first image or default thumbnail -->
				<div class="the_excrept"><font size="3" face="Verdana">BpmnName:</font>
            		<font size="2" ><a href="#" name="bpmnName"><%=uploadbpmn.getBpmnName() %></a></font>
            	</div>
            	<div class="the_excrept"><font size="3" face="Verdana">Description:</font>
            		<font size="2" ><a href="/repository/bpmn/<%=userName%>/<%=uploadbpmn.getBpmnName()%>.bpmn" target="_blank">BPMN file</a></font>
            	</div>
        		<div class="clear"></div> 
				<table width="600px"><tr><td width="200px" align="right"><!--  <a href="javascript:publish(<%=uploadbpmn.getBpmnId() %>);">Publish</a>  --></td><td width="200px" align="center"></td><td align="left"><a href="javascript:edit(<%=uploadbpmn.getBpmnId() %>);">Edit</a></td></tr></table>		
        		<div class="excrept_data">
            		<div class="excrept_right">
            		<div class="options">
            		<form action="UploadBpmnOperate" method="post" onsubmit="return bpmnOperate()">
				    	<input type="hidden" name="bpmnId" value="<%=uploadbpmn.getBpmnId()%>" ></input>
						<input style="width:80px" type="button" value="Use Now" id="implementid" onclick="showDialog2(this)"></input>
						<input style="width:80px" type="button" value="Remove" id="removeid" onclick="removeUpBPMN(<%=uploadbpmn.getBpmnId()%>)"></input>
					</form>
                	</div>
            		</div>
        			<div class="clear"></div>
...
<%
 ...
 <!-- content end here -->
    ...
    <div id="implementDialog" class="hidden dialog">
		...
		<div class="content">
			<form action="BpmnOperate" method="post"></form>
		</div>
	</div>
